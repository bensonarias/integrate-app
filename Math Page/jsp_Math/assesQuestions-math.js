// Array of Questions


// Counting Numbers
const math1 = [{
    q: 'Fill in the blank <br> 11, 12, ___, 14, 15',
    options:['13','23','10','33'],
    answer:0
},
{
    q: 'What number is this? <br> 50',
    options:['fifty','fifteen','thirty','sixty'],
    answer:0
},
{
    q: 'This is skip counting by ______? <br> 3, 6, 9, 12, 15',
    options:['3','2','4','5'],
    answer:0
},
{
    q: 'Fill in the blanks. Skip Counting by (10) <br> 10, ___, 30, ___, 50',
    options:['20 and 30','20 and 60','20 and 10','20 and 40	'],
    answer:3
},
{
    q: 'Fill in the blank 34 __ 43',
    options:['<','>','='],
    answer:0
},
{
    q: 'Fill in the blank 10 __ 10',
    options:['>','<','='],
    answer:2
},
{
    q: 'What is the place value of 6 in 216?',
    options:['ones','tens','hundreds'],
    answer:0
},
{
    q: 'What is the place value of 1 in 125?',
    options:['ones','tens','hundreds'],
    answer:2
},
{
    q: 'What is the equivalent of first?',
    options:['1th','4th','1st','1nd'],
    answer:2
},
{
    q: 'Fill in the blank 1st, ___, 3rd, 4th, 5th',
    options:['2st','2','2nd','2rd'],
    answer:2
}

]

// Addition
const math2 = [{
  q: '<img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"> <img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"> + <img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"> <img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"> <img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple">',
    options:['<img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"> <img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple">', '<img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple">','<img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple"><img src="/Math Page/img-math/addition/pineapple.png" alt="one pineapple">'],
    answer:1
},
{
    q: '<img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"> + <img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish">',
    options:['<img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish">', '<img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish">', '<img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish"><img src="/Math Page/img-math/addition/fish.png" alt="one fish">'],
    answer:1
},
{
    q: '_ + 4 = 9',
    options:['5', '6', '7'],
    answer:0
},
{
    q: '5 <br> + 3 = _',
    options:['6', '8', '7'],
    answer:2
},
{
    q: '6 <br> + 2 = ',
    options:['8', '7', '9'],
    answer:0
},
{
    q: '77 <br> + 13 = ',
    options:['90', '80', '100'],
    answer:0
},
{
   q: '67 <br> + 8 = ',
    options:['85', '65', '75'],
    answer:2
},
{
    q: 'Lauren has 3 apples and Marty has 5 apples. How many apples do they have when they combine apples?',
   options:['8', '9', '7'],
    answer:1
},
{
    q: 'I need ten candy. I found two candies in my pocket. How many more candies do I need to buy at the store? ',
   options:['10', '11', '12'],
    answer:2
},
{
    q: 'Which of the following DOES NOT belong to the group?',
    options:[],
    answer:3
}];

// Subtraction
const math3 = [{
    q: '50 – 25 = __',
    options:['50','25','30','10'],
    answer:1
},
{
    q: '100 – 100 = __',
    options:['5','10','0','15'],
    answer:2
},
{
    q: '78 – 59 = __',
    options:['19','33','57','47'],
    answer:0
},
{
    q: '__ - 55 = 45',
    options:['75','100',' 25',' 95'],
    answer:1
},
{
    q: 'Base on the image that has been shown, what is the correct answer? <br> <img src="/Math Page/img-math/subtraction/number5.png" alt="question 5">',
    options:['7','8','4','5'],
    answer:2
},
{
    q: 'Base on the image that has been shown, what is the correct answer? <br> <img src="/Math Page/img-math/subtraction/number6.png" alt="question 6">',
    options:['2','3','4','5'],
    answer:0
},
{
    q: '20 – 5 = ____',
    options:['5','15','10','20'],
    answer:0
},
{
    q: '99 – 10 = ___',
    options:['81','88','90','89'],
    answer:3
},
{
    q: '__ - 5 = 10',
    options:['10','15','20','5'],
    answer:1
},
{
    q: '50 – 25 = ___',
    options:['15','25','30','35'],
    answer:1
}];

// Fraction
const math4 = [{
    q: 'Which of the following is divided into equal parts?',
    options:['<img src="/Math Page/img-math/fraction/1.1.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/1.2.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/1.3.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/1.4.png" alt="fraction">',],
    answer:2
},
{
    q: 'Which of the fofollowing is NOT divided into equal parts? ',
    options:['<img src="/Math Page/img-math/fraction/2.1.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/2.2.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/2.3.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/2.4.png" alt="fraction">',],
    answer:3
},
{
    q: 'What is the fraction corresponded to the figure below? <br> <img src="/Math Page/img-math/fraction/3.png" alt="fraction"> ',
    options:['6/8','2/8','2/6'],
    answer:1
},
{
    q: 'Which of the following is the figure that corresponds to ¾?',
    options:['<img src="/Math Page/img-math/fraction/4.1.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/4.2.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/4.3.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/4.4.png" alt="fraction">',],
    answer:2
},
{
    q: 'Which of the following is corresponded to 6/8? ',
    options:['<img src="/Math Page/img-math/fraction/5.1.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/5.2.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/5.3.png" alt="fraction">'],
    answer:1
},
{
    q: 'What is the numerator and the denominator of the figure below? <br> <img src="/Math Page/img-math/fraction/6.png" alt="fraction">',
    options:['3/6','3/5','2/6'],
    answer:1
},
{
    q: 'Which of the following is correct on writing 1/3 fraction into words? <br> <img src="/Math Page/img-math/fraction/7.png" alt="fraction"> ',
    options:['One-three','One-thirds','One-third'],
    answer:2
},
{
    q: 'Choose the correct way of writing 2/5 fraction into words. <br> <img src="/Math Page/img-math/fraction/8.png" alt="fraction"> ',
    options:['Two-fifth','Two-five','Two-fifths'],
    answer:2
},
{
    q: 'Which of the following is incorrect? ',
    options:['<img src="/Math Page/img-math/fraction/9.1.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/9.2.png" alt="fraction">', '<img src="/Math Page/img-math/fraction/9.3.png" alt="fraction">'],
    answer:0
},
{
    q: 'Which of the following is correct on writing ¼ into words? <br> <img src="/Math Page/img-math/fraction/10.png" alt="fraction">',
    options:['One-four','One-fourths','One-fourth'],
    answer:2
}];

// Money
const math5 = [{
q: 'Identify the amount of Money <br> <img src="/Math Page/img-math/money/20.png" alt="20 pesos">',
    options:['20 pesos','200 pesos','1000 pesos','25 centavos'],
    answer:0
},
{
    q: 'What is the amount of the picture below? <br> <img src="/Math Page/img-math/money/10.png" alt="10 pesos">',
    options:['10 pesos','500 pesos','5 pesos','1 peso'],
    answer:0
},
{
    q: 'How much money is in the image? <br> <img src="/Math Page/img-math/money/1cent.png" alt="1cent">',
    options:['20 pesos','100 pesos','1000 pesos','25 centavos'],
    answer:3
},
{
    q: 'What currency is this? This is the currency of Japan. <br> <img src="/Math Page/img-math/money/yen.png" alt="yen">',
    options:['Pesos','Dollars', 'Yen', 'Euro'],
    answer:2
},
{
    q: 'How much money is in the image? <br> <img src="/Math Page/img-math/money/100.png" alt="100 pesos"> ',
    options:['1000 pesos','1 centavo','5 centavos','100 pesos'],
    answer:3
},
{
    q: 'What country has a currency called Dollar? ',
    options:['China','USA','Japan','Philippines'],
    answer:1
},
{
    q: 'Give money to complete the amount(516.30)',
    options:['One 500, One 5, Three 1, One 25 centavo','B.Three 100, One 5, Three 1, One 25 centavo','One 500, Three 5, One 1, Three 10 centavos', 'One 500'],
    answer:2
},
{
    q: 'Give money to complete the amount 210',
    options:['One 100','Two 100, 3 Five','One 200, One 10', 'One 200, One 50'],
    answer:2
},
{
    q: 'Troy have 100 pesos from his pocket. He receives another 100 pesos from his mother and his father gave him 50 pesos. How much money Troy have in total?',
   options:['5 pesos','25 pesos','250 pesos','25000 pesos'],
    answer:2
},
{
    q: 'Joy have 50 pesos and she will buy a meal that costs 25 pesos. How much money will she receive as change?',
    options:['5 pesos','25 pesos','250 pesos','25000 pesos'],
    answer:1
}];

// Shapes
const math6 = [{
q: 'Form of an object or its external boundary, outline, or external surface.',
    options:['Colors','Lines','Shapes','Textures'],
    answer:2
},
{
    q: 'What is an example of Circle?',
    options:['<img src="/Math Page/img-math/shapes/ball.png" alt="ball"></img>', '<img src="/Math Page/img-math/shapes/watermelon.png" alt="watermelon"></img>', '<img src="/Math Page/img-math/shapes/window.png" alt="window"></img>', '<img src="/Math Page/img-math/shapes/glass.png" alt="glass"></img>' ],
    answer:0
},
{
    q: 'What is an example of Square?',
    options:['<img src="/Math Page/img-math/shapes/warning.png" alt="warning"></img>', '<img src="/Math Page/img-math/shapes/seeds.png" alt="seeds"></img>', '<img src="/Math Page/img-math/shapes/glass.png" alt="glass"></img>', '<img src="/Math Page/img-math/shapes/board.png" alt="board"></img>' ],
    answer:2
},
{
    q: 'What is an example of Triangle?',
    options:['<img src="/Math Page/img-math/shapes/square.png" alt="square"></img>', '<img src="/Math Page/img-math/shapes/pizza.png" alt="pizza"></img>', '<img src="/Math Page/img-math/shapes/wheel.png" alt="wheel"></img>', '<img src="/Math Page/img-math/shapes/window.png" alt="window"></img>' ],
    answer:1
},
{
    q: 'What is an example of Rectangle?',
    options:['<img src="/Math Page/img-math/shapes/seeds.png" alt="seed"></img>', '<img src="/Math Page/img-math/shapes/glass.png" alt="glass"></img>', '<img src="/Math Page/img-math/shapes/board.png" alt="board"></img>', '<img src="/Math Page/img-math/shapes/cone.png" alt="cone"></img>' ],
    answer:2
},
{
    q: 'Look at the picture, what shape is the Tree? <br> <img src="/Math Page/img-math/shapes/house.png" alt="house"></img>',
    options:['Trees are Circle','Trees are Triangle','Trees are Square','It has no shape'],
    answer:1
},
{
    q: 'Look at the picture, what shape is the road? <br> <img src="/Math Page/img-math/shapes/house.png" alt="house"></img>',
   options:['The road is Circle','The road is Triangle','The road is Square','The road is Rectangle'],
    answer:3
},
{
    q: ': Look at the picture, what shape is the windows? <br> <img src="/Math Page/img-math/shapes/house.png" alt="house"></img>',
   options:['The windows are Circle','The windows are Square','The windows are Triangle','The windows are Rectangle'],
    answer:1
},
{
    q: 'Which of the following DOES NOT belong to the group?',
    options:['<img src="/Math Page/img-math/shapes/shape1.png" alt="shape"></img>', '<img src="/Math Page/img-math/shapes/shape2.png" alt="shape"></img>', '<img src="/Math Page/img-math/shapes/shape3.png" alt="shape"></img>', '<img src="/Math Page/img-math/shapes/shape4.png" alt="shape"></img>' ],
    answer:2
},
{
    q: 'Which of the following DOES NOT belong to the group?',
    options:['<img src="/Math Page/img-math/shapes/shape5.png" alt="shape"></img>', '<img src="/Math Page/img-math/shapes/shape6.png" alt="shape"></img>', '<img src="/Math Page/img-math/shapes/shape7.png" alt="shape"></img>', '<img src="/Math Page/img-math/shapes/shape8.png" alt="shape"></img>' ],
    answer:3
}];

// Time
const math7 = [{
    q: 'What time is shown in the image? <br> <img src="/Math Page/img-math/time/time_1.png" alt="time"></img>',
    options: ['7:35','2:30','5:35','3:35'],
    answer: 3  
},
{
    q: 'What time is shown in the image? <br> <img src="/Math Page/img-math/time/time_2.png" alt="time"></img>',
    options: ['5:45','5:35','5:15','5:50'],
    answer: 0  
},
{
    q: 'What time is shown in the image? <br> <img src="/Math Page/img-math/time/time_3.png" alt="time"></img>',
    options: ['11:25','11:55','11:15','11:30'],
    answer: 3 
},
{
    q: 'What time is shown in the image? <br> <img src="/Math Page/img-math/time/time_4.png" alt="time"></img>',
    options: ['5:25','1:20','1:15','1:25'],
    answer: 3  
},
{
    q: 'What time is shown in the image? <br> <img src="/Math Page/img-math/time/time_5.png" alt="time"></img>',
    options: ['9:00','9:30','9:50','9:10'],
    answer: 3 
},
{
    q: 'What time is shown in the image? <br> <img src="/Math Page/img-math/time/time_6.png" alt="time"></img>',
    options: ['7:50','1:50','2:50','8:50'],
    answer: 3  
},
{
    q: 'What time is shown in the image? <br> <img src="/Math Page/img-math/time/time_7.png" alt="time"></img>',
    options: ['7:00','7:25','7:30','7:15'],
    answer: 1  
},
{
    q: 'What time is shown in the image? <br> <img src="/Math Page/img-math/time/time_8.png" alt="time"></img>',
    options: ['12:00','12:45','12:15','12:40'],
    answer: 2 
},
{
    q: 'What time is shown in the image? <br> <img src="/Math Page/img-math/time/time_9.png" alt="time"></img>',
    options: ['11:40','12:40','10:40','5:40'],
    answer: 2
},
{
    q: 'What time is shown in the image? <br> <img src="/Math Page/img-math/time/time_10.png" alt="time"></img>',
    options: ['3:45','6:45','1:45','2:45'],
    answer: 1 
}];

// Measurement
const math8 = [{
    q: 'What is lighter? <br> <img src="/Math Page/img-math/measurement/1.png" alt="measure"></img>',
    options: ['Fruits','Junkfoods','Both','None of them'],
    answer: 1
},
{
    q: 'Who is the heaviest Monkey? <br> <img src="/Math Page/img-math/measurement/2.png" alt="measure"></img>',
    options: ['The monkey with Banana','Both Monkeys','The monkey with many banana','None of them'],
    answer: 2
},
{
    q: 'What is lighter? <br> <img src="/Math Page/img-math/measurement/3.png" alt="measure"></img>',
    options: ['The boy carrying with bag','Both the boy and the girl','The girl carrying a box','None of them'],
    answer: 2
},
{
    q: 'What is the heavier? <br> <img src="/Math Page/img-math/measurement/4.png" alt="measure"></img>',
    options: ['Elephant','Lion cub','Both of them','None of them'],
    answer: 0
},
{
    q: 'What is lighter? <br> <img src="/Math Page/img-math/measurement/5.png" alt="measure"></img>',
    options: ['The girl with one ball','The girl with two ball','Both','None of them'],
    answer: 0
},
{
    q: 'Which pencil is longer? <br> <img src="/Math Page/img-math/measurement/6.png" alt="measure"></img>',
    options: ['The pencil on the right ','The pencil on the left','Both of them','None of them'],
    answer: 0
},
{
    q: 'Which pencil is shorter? <br> <img src="/Math Page/img-math/measurement/7.png" alt="measure"></img>',
    options: ['The pencil on the right ','The pencil on the left','Both of them','None of them'],
    answer: 0
},
{
    q: 'Which have the most capacity? <br> <img src="/Math Page/img-math/measurement/8.png" alt="measure"></img>',
    options: ['The glass','The bottle','Both','None of them'],
    answer: 1
},
{
    q: 'Which have the less capacity? <br> <img src="/Math Page/img-math/measurement/9.png" alt="measure"></img>',
    options: ['The glass','The bottle','Both','None of them'],
    answer: 0
},
{
    q: 'Who is lighter? <br> <img src="/Math Page/img-math/measurement/10.png" alt="measure"></img>',
    options: ['The man on the right','The man on the left','Both','None of them'],
    answer: 0
}];

